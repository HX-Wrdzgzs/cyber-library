...
def is_valid_isbn10(value: str) -> bool:
    value = compact(value)
    if len(value) != 10 or not re.fullmatch(r"\d{9}[\dX]", value): return False
    return sum((10 - i) * (10 if ch == "X" else int(ch)) for i, ch in enumerate(value)) % 11 == 0


def is_valid_isbn13(value: str) -> bool:
    value = compact(value)
    if len(value) != 13 or not value.isdigit() or value[:3] not in {"978", "979"}: return False
...
def isbn10_to_isbn13(value: str) -> str:
    value = compact(value)
    if not is_valid_isbn10(value): raise InvalidISBN(f"Invalid ISBN-10: {value}")
    body = "978" + value[:9]
...
def normalize_isbn(value: str) -> str:
    value = compact(value)
    if len(value) == 10: return isbn10_to_isbn13(value)
    if len(value) == 13 and is_valid_isbn13(value): return value
