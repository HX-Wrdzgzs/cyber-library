...
class OpenLibraryClient:
    """Low-volume, human-triggered Open Library lookup client.

    Bulk catalog creation must use Open Library data dumps instead of repeatedly
    calling this API.
    """

    def __init__(self, cache: JsonCache | None = None, contact: str | None = None, timeout: float = 12.0) -> None:
        self.cache = cache
        self.timeout = timeout
        self.contact = (contact or os.getenv("CYBER_LIBRARY_CONTACT", "")).strip()
        self.user_agent = "CyberLibrary/0.1.0" + (f" ({self.contact})" if self.contact else "")
    ...
    def lookup_isbn(self, isbn: str) -> BookRecord:
        ...
        ekey = str(eraw.get("key") or f"/isbn/{isbn13}")
        eid = ekey.rsplit("/", 1)[-1]
        ...
        if wraw is not None and wkey:
            wid = wkey.rsplit("/", 1)[-1]
            work = Work(
                id=f"openlibrary:work:{wid}",
                title=str(wraw.get("title") or eraw.get("title") or ""),
        ...
        ids = {"isbn13": [isbn13]}
        if eraw.get("isbn_10"):
            ids["isbn10"] = [str(x).replace("-", "") for x in eraw["isbn_10"]]
        if eraw.get("isbn_13"):
            ids["isbn13"] = [str(x).replace("-", "") for x in eraw["isbn_13"]]

        edition = Edition(
            id=f"openlibrary:edition:{eid}",
            work_id=f"openlibrary:work:{wid}" if wid else None,
            title=str(eraw.get("title") or ""),
            language=language,
            publishers=[str(x) for x in (eraw.get("publishers") or [])],
            publish_date=eraw.get("publish_date"),
            page_count=eraw.get("number_of_pages"),
            identifiers=ids,
        ...
        return BookRecord(
            ...
            analysis=Analysis(
                ...
                confidence=1.0,
                sources=sources,
                warnings=["No AI interpretation has been generated for this record."],
            ),
            provenance=[
                Provenance(
                    source="Open Library",
