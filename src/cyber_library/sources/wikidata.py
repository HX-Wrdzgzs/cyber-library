...
class WikidataAdapter:
    ...
    def _query(self, sparql: str, cache_key: str) -> dict:
        ...
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "CyberLibrary/1.2 (+https://github.com/HX-Wrdzgzs/cyber-library)"
                + (f" ({self.contact})" if self.contact else ""),
            },
        )
        try:
            ...
            detail = ""
            try:
                detail = exc.read(1600).decode("utf-8", "replace")
            except Exception:
                pass
    ...
    def reconcile_isbn(self, isbn: str) -> list[SourceMatch]:
        ...
        isbn10 = raw if is_valid_isbn10(raw) else None
        branches = [
            "{ ?item wdt:P212 ?matched . "
            f'FILTER(REPLACE(UCASE(STR(?matched)), "-", "") = "{isbn13}") }}'
        ]
        if isbn10:
            branches.append(
                "{ ?item wdt:P957 ?matched . "
                f'FILTER(REPLACE(UCASE(STR(?matched)), "-", "") = "{isbn10}") }}'
            )
        union = " UNION ".join(branches)
        sparql = f"""
SELECT DISTINCT ?item ?itemLabel ?itemDescription ?isbn13 ?isbn10 WHERE {{
  {union}
...
""".strip()
        ...
        for binding in bindings:
            ...
            if not qid.startswith("Q") or not qid[1:].isdigit() or qid in seen:
                continue
            seen.add(qid)
            identifiers: dict[str, list[str]] = {"wikidata": [qid]}
            value13 = self._binding_value(binding, "isbn13")
            value10 = self._binding_value(binding, "isbn10")
            if value13:
                identifiers["isbn13"] = [compact(value13)]
            if value10:
                identifiers["isbn10"] = [compact(value10)]
            matches.append(
                SourceMatch(
                    source=self.name,
                    source_id=qid,
                    url=f"https://www.wikidata.org/wiki/{qid}",
                    label=self._binding_value(binding, "itemLabel"),
                    description=self._binding_value(binding, "itemDescription"),
                    confidence=1.0,
                    identifiers=identifiers,
                    metadata={"matched_isbn13": isbn13, "matched_isbn10": isbn10},
                )
            )
        return matches
